...
if __name__ == '__main__':
    ...
    print "[Logistic Regression's Best Parameter]", clf_cv.best_params_
    clf = LogisticRegression()
    clf.set_params(**clf_cv.best_params_)
    del clf_cv
    clf.fit(transformated_train_features, y_train)
    [[TP,FP],[FN,TN]] = metrics.confusion_matrix(y_test, clf.predict(transformated_test_features))
    ...
    # 4. svm using transformated features
    parameters = [{'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000],
                   'loss':['hinge'],
                   'penalty':['l2'],
                   'tol':[1.0e-3],
    ...
    print "[SVM's Best Parameter]", clf_cv.best_params_
